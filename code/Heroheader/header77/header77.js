...
export type Header77Props = React.ComponentPropsWithoutRef<"section"> & Props;
...
export const Header77 = (props: Header77Props) => {
  ...
  return (
    <header className="grid grid-cols-1 items-center gap-y-16 pt-16 md:pt-24 lg:grid-cols-2 lg:pt-0">
        ...
        <div className="grid shrink-0 grid-cols-1 gap-y-4">
          <div className="ml-[-8.5%] grid w-full animate-marquee-horizontally auto-cols-fr grid-cols-2 gap-4 self-center">
            {[...new Array(2)].map((e, index) => (
              <div key={`part-1-${index}`} className="grid w-full grid-flow-col gap-4">
                {images.map((image, imageIndex) => (
                  <div
                    key={`${image.src}-${index}-${imageIndex}`}
          ...
          <div className="grid w-full animate-marquee-horizontally grid-cols-2 gap-4 self-center">
            {[...new Array(2)].map((e, index) => (
              <div key={`part-2-${index}`} className="grid w-full grid-flow-col gap-4">
                {images.map((image, imageIndex) => (
                  <div
                    key={`${image.src}-${index}-${imageIndex}`}
                    className="relative w-[60vw] pt-[75%] sm:w-[18rem] md:w-[26rem]"
...
const Header77Defaults: Header77Props = {
