...
export type Gallery13Props = React.ComponentPropsWithoutRef<"section"> & Props;
      ...
      <Carousel setApi={setApi} opts={options} className="overflow-hidden">
        <CarouselContent className="ml-0">
          {images.map((image, index) => (
            <CarouselItem key={`${image.src}-${index}`} className="relative h-dvh pl-0">
              <img
                src={image.src}
                alt={image.alt}
...
const Gallery13Defaults: Gallery13Props = {
